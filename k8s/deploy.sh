...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo -e "${GREEN}The frontend is configured to connect to the backend at: http://localhost:8080${NC}"
    echo ""
    echo -e "${GREEN}Alternative deployment methods:${NC}"
    echo -e "${YELLOW}Using Helm:${NC}"
    echo "  cd .. && helm install ccf-ui ./helm-chart/ccf-ui -f ./helm-chart/ccf-ui/values-dev.yaml"
    echo ""
    echo -e "${YELLOW}Using ArgoCD:${NC}"
    echo "  kubectl apply -f ../argocd/ccf-ui-application.yaml"
else
