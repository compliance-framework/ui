  ...
  <div class="mb-6">
    ...
    <div class="space-y-3">
      ...
      >
        ...
        <div class="space-y-3">
          <div>
            <label class="inline-block pb-1 text-sm dark:text-slate-300">Select Task</label>
            ...
            <div class="relative">
              <select
                ...
                @change="updateTaskSelection(index, ($event.target as HTMLSelectElement).value)"
            ...
            <!-- Selected Task Info -->
            <div v-if="dependency.taskUuid && getSelectedTask(dependency.taskUuid)" class="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded border">
              <div class="text-sm font-medium dark:text-slate-300">
                {{ getSelectedTask(dependency.taskUuid)?.title }}
              </div>
              <div v-if="getSelectedTask(dependency.taskUuid)?.description" class="text-sm text-gray-600 dark:text-slate-400 mt-1">
...
watch(() => dependencies.value.length, (newLength) => {
...
const fetchTasks = async () => {
  ...
  try {
    console.log('TaskDependencyManager: Fetching tasks for assessment plan:', props.assessmentPlanId)
    ...
    console.log('TaskDependencyManager: Available tasks set to:', availableTasks.value)
  } catch (err) {
    ...
    try {
      ...
      const fallbackResponse = await assessmentPlanStore.get(props.assessmentPlanId)
      console.log('TaskDependencyManager: Fallback response:', fallbackResponse)
      availableTasks.value = fallbackResponse.data?.tasks || []
      console.log('TaskDependencyManager: Fallback tasks set to:', availableTasks.value)
      error.value = '' // Clear error if fallback works
    } catch (fallbackErr) {
      console.error('TaskDependencyManager: Fallback also failed:', fallbackErr)
    }
  } finally {
    loading.value = false
  }
...
const getFilteredTasks = (index: number): Task[] => {
  ...
  let filteredTasks = availableTasks.value.filter(task =>
    task.uuid !== props.currentTaskUuid
  )
