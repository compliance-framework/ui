<template>
  <div class="px-8 py-6 max-w-4xl mx-auto">
    <h2 class="text-xl font-semibold mb-6 dark:text-slate-300">
      Manage Inventory Item Relationships
    </h2>
    ...
    <div v-else>
      ...
      <div v-if="activeTab === 'props'" class="space-y-3">
        ...
        <div
          v-for="(prop, index) in workingItem.props"
          :key="index"
      ...
      <div v-if="activeTab === 'links'" class="space-y-3">
        ...
        <div
          v-for="(link, index) in workingItem.links"
          :key="index"
          class="border border-gray-200 dark:border-slate-600 rounded-lg p-4"
        >
      ...
      <div v-if="activeTab === 'responsible-parties'" class="space-y-3">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-medium dark:text-slate-300">
            Responsible Parties
          </h3>
          <button
        ...
        <div
          v-for="(party, index) in workingItem.responsibleParties"
          :key="index"
          class="border border-gray-200 dark:border-slate-600 rounded-lg p-4"
        >
          <div class="space-y-4">
      ...
      <div v-if="activeTab === 'implemented-components'" class="space-y-3">
        ...
        <div
          v-for="(component, index) in workingItem.implementedComponents"
          :key="index"
          class="border border-gray-200 dark:border-slate-600 rounded-lg p-4"
        >
          <div class="space-y-4">
...
const activeTab = ref<
  'props' | 'links' | 'responsible-parties' | 'implemented-components'
>('props');
...
function getAvailableCount(type: string): number {
  switch (type) {
    case 'props':
      return workingItem.props?.length || 0;
    case 'links':
      return workingItem.links?.length || 0;
    case 'responsible-parties':
      return workingItem.responsibleParties?.length || 0;
    case 'implemented-components':
      return workingItem.implementedComponents?.length || 0;
    default:
      return 0;
  }
}
...
function addResponsibleParty() {
  ...
  workingItem.responsibleParties.push({
    roleId: '',
    partyUuids: [],
    remarks: '',
  });
...
function updatePartyUuids(index: number) {
  if (workingItem.responsibleParties && workingItem.responsibleParties[index]) {
    const uuids = partyUuidStrings.value[index]
      .split(',')
      .map((uuid) => uuid.trim())
      .filter((uuid) => uuid);
...
function addImplementedComponent() {
  if (!workingItem.implementedComponents)
    workingItem.implementedComponents = [];
...
onMounted(() => {
  ...
  if (workingItem.responsibleParties) {
    partyUuidStrings.value = workingItem.responsibleParties.map(
      (party) => party.partyUuids?.join(', ') || '',
    );
  }
});
</script>
