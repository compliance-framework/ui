...
async function createAsset(): Promise<void> {
  ...
  try {
    ...
    const filteredProps = (asset.value.props || []).filter(
      (prop) =>
        prop.name?.trim() ||
        prop.value?.trim() ||
        prop.class?.trim() ||
    ...
    const filteredLinks = (asset.value.links || []).filter(
      (link) => link.href?.trim() || link.rel?.trim() || link.text?.trim(),
    );
    ...
    const assetData: AssessmentAsset = {
      ...
      components:
        asset.value.components && asset.value.components.length > 0
