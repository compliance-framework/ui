  ...
  <div>
    <div class="flex items-center justify-between mb-4">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-slate-400">
          System roles granted to this group. Members inherit these roles.
        </p>
      </div>
      <PermissionGate
    ...
    <div v-else-if="!roles?.length" class="text-center py-10">
      ...
      <p class="text-sm text-gray-400 dark:text-slate-500 mt-1">
        Assign a role to grant it to every member of this group.
      </p>
    </div>
    ...
    >
      <form @submit.prevent="assignRole" class="pt-1 w-full sm:w-96 space-y-4">
        <div>
          <Label for="assign-group-role">Role</Label>
          <Select
            id="assign-group-role"
...
const {
  ...
  error: rolesError,
  execute: fetchRoles,
} = useDataApi<CCFRoleAssignment[]>(
  `/api/admin/groups/${props.groupId}/roles`,
  {},
  { immediate: false },
);

// Roles already granted to the group — exclude them from the picker so a redundant grant
// doesn't 409.
const assignedRoleNames = computed(
  () => new Set((roles.value ?? []).map((r) => r.roleName)),
);

const availableRoles = computed(() =>
  MANIFEST_ROLES.filter((r) => !assignedRoleNames.value.has(r.name)),
);
...
async function removeRole(role: CCFRoleAssignment) {
  try {
    await axiosInstance.delete(`/api/admin/role-assignments/${role.id}`);
    await fetchRoles();
