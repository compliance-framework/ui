<template>
  <PageHeader> All Subjects </PageHeader>
  ...
  >
    <table class="table-auto w-full rounded-full dark:text-slate-300 text-left">
      ...
      <tbody v-if="isLoading">
        <tr>
          <td
            class="py-2 px-4 font-medium text-wrap"
            aria-live="polite"
            role="status"
          >
            Loading ...
      ...
      <tbody v-else-if="error">
        <tr>
          <td
            class="py-2 px-4 font-medium text-wrap text-red-500"
            aria-live="assertive"
            role="alert"
          >
      ...
      <tbody v-else>
        ...
        >
          ...
          <td class="py-2 px-4 w-[1%]">
            <router-link
              :to="{ name: 'admin-subject-crud', params: { id: subject._id } }"
            >
              <button
...
const {
  ...
  isLoading,
  error,
} = useDataApi<Subject[]>('/api/subjects');
</script>
