<template>
  <template v-if="planOfActionAndMilestones">
    <div class="space-y-6">
      ...
      <!-- POAM Items Summary -->
      <div v-if="poamItems && poamItems.length > 0" class="bg-white dark:bg-slate-900 border border-ccf-300 dark:border-slate-700 rounded-lg p-6">
...
import type { PlanOfActionAndMilestones, PoamItem, Observation, Risk } from '@/stores/plan-of-action-and-milestones.ts'
...
const { data: planOfActionAndMilestones } = useDataApi<PlanOfActionAndMilestones>(`/api/oscal/plan-of-action-and-milestones/${route.params.id}`)
const { data: poamItems } = useDataApi<PoamItem[]>(`/api/oscal/plan-of-action-and-milestones/${route.params.id}/poam-items`)
const { data: observations } = useDataApi<Observation[]>(`/api/oscal/plan-of-action-and-milestones/${route.params.id}/observations`)
const { data: risks } = useDataApi<Risk[]>(`/api/oscal/plan-of-action-and-milestones/${route.params.id}/risks`)
const { data: fullPOAM, execute: executeJSON } = useDataApi<PlanOfActionAndMilestones>(`/api/oscal/plan-of-action-and-milestones/${route.params.id}/full`,
  null,
  { immediate: false }
)
...
async function downloadJson(): Promise<void> {
  try {
    ...
    const dataStr = JSON.stringify(decamelizeKeys(fullPOAM.value!, { separator: '-', deep: true}), null, 2)
...
function handleMetadataSaved(updatedMetadata: Metadata) {
  ...
  if (planOfActionAndMilestones.value && planOfActionAndMilestones.value.metadata) {
