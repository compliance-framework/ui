<template>
  ...
  <template v-else-if="error">
    <p class="text-red-500">
      Error loading assessment plan: {{ (error as AxiosError<{message: string}>).response?.data.message || 'Unknown error' }}
    </p>
  </template>
...
const { data: assessmentPlan, error, isLoading } = useDataApi<AssessmentPlan>(`/api/oscal/assessment-plans/${toValue(id)}`)
watch(error, () => {
  if (error.value) {
