...
import type { Task, AssessmentAsset, AssessmentPlan, AssessmentSubject } from '@/stores/assessment-plans.ts'
...
const { data: subjects, execute: refreshSubjects } = useDataApi<AssessmentSubject[]>(`/api/oscal/assessment-plans/${route.params.id}/assessment-subjects`, null, { immediate: false })
const { data: assets, execute: refreshAssets } = useDataApi<AssessmentAsset[]>(`/api/oscal/assessment-plans/${route.params.id}/assessment-assets`, null, { immediate: false })
