<template>
  ...
  <div v-else>
    ...
    <PageSubHeader
      >Specify how controls are implemented across the business</PageSubHeader
    >
    ...
    <div class="space-y-6">
      ...
      >
        ...
        <div v-else-if="error" class="text-center py-4">
          <p class="text-red-500">
            Error loading control implementation: {{ error }}
          </p>
        </div>
      ...
      >
        ...
        <div
          v-if="controlImplementation.implementedRequirements?.length"
          class="space-y-6"
        >
          <div
              ...
              <div class="space-y-3">
                ...
                >
                  ...
                  <div v-if="statement.byComponents?.length" class="mt-2">
                    <span
                      class="text-xs font-medium text-gray-700 dark:text-slate-400"
                      >By Components:</span
                    >
  ...
  <Modal
    :show="showEditControlImplementationModal && controlImplementation !== null"
...
watch(profile, () => {
  ...
  if (!catalog.value?.uuid) {
    ...
    useFetch(
      new Request(`/api/oscal/profiles/${profile.value.uuid}/resolved`),
    )
...
const deleteRequirement = async (requirement: ImplementedRequirement) => {
  ...
  try {
    await sspStore.deleteImplementedRequirement(
...
const handleRequirementSaved = (updatedRequirement: ImplementedRequirement) => {
  if (controlImplementation.value) {
    const index = controlImplementation.value.implementedRequirements.findIndex(
      (r) => r.uuid === updatedRequirement.uuid,
    );
    if (index !== -1) {
    ...
    if (reqIndex !== -1) {
      ...
      if (requirement.statements) {
        const statementIndex = requirement.statements.findIndex(
          (s) => s.uuid === updatedStatement.uuid,
        );
        if (statementIndex !== -1) {
...
const handleStatementByComponentSaved = (updatedByComponent: ByComponent) => {
  if (
    ...
    if (reqIndex !== -1) {
      ...
      if (requirement.statements) {
        const statementIndex = requirement.statements.findIndex(
          (s) => s.uuid === editingStatement.value?.uuid,
        );
        if (statementIndex !== -1) {
