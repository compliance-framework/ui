  ...
  >
    <table class="table-auto w-full rounded-full dark:text-slate-300 border-collapse">
      ...
      <tbody v-else-if="error">
        <tr>
          <td class="py-3 px-4 font-medium text-wrap text-red-500">Error loading profiles</td>
        </tr>
      </tbody>
      <tbody v-else>
      <tr
...
const { data: profiles, isLoading, error } = useDataApi<Profile[]>('/api/oscal/profiles');
