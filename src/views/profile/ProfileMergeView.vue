  ...
  <PageCard v-if="merge">
    ...
    <form class="space-y-4">
      <div>
        ...
        <select
          v-model="mergeType"
          class="w-full rounded border border-ccf-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
        >
      ...
      <div>
        <label class="inline-flex items-center cursor-pointer">
          <input
            type="checkbox"
            v-model="combineEnabled"
            class="form-checkbox h-5 w-5 text-blue-600 rounded border-ccf-300 dark:accent-slate-700 dark:border-slate-700 focus:ring-blue-400"
          />
      ...
      <div v-if="combineEnabled && merge.combine">
        <label
          class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1"
          >Combine Method</label
        >
        <select
          v-model="merge.combine.method"
          class="w-full rounded border border-ccf-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
        >
...
const { data: updatedMerge, execute: updateMergeExecute } = useDataApi<Merge>(
  `/api/oscal/profiles/${id}/merge`,
...
watch(
  ...
  () => {
    if (merge.value) {
      mergeType.value = merge.value.asIs
        ? 'asIs'
        : merge.value.flat
          ? 'flat'
          : 'custom';
  ...
  try {
    ...
    toast.add({
      ...
      detail:
        ...
        'An error occurred while updating merge settings.',
      life: 3000,
    });
  }
}
