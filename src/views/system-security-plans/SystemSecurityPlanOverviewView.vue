...
const { data: systemSecurityPlan } = useDataApi<SystemSecurityPlan>(`/api/oscal/system-security-plans/${route.params.id}`);

const { data: systemCharacteristics } = useDataApi<SystemCharacteristics>(`/api/oscal/system-security-plans/${route.params.id}/system-characteristics`);

const { execute: executeAttachedProfile } = useDataApi<Profile>(`/api/oscal/system-security-plans/${route.params.id}/profile`, {
  method: 'GET',
}, { immediate: false });
const { execute: attachProfile } = useDataApi<void>(`/api/oscal/system-security-plans/${route.params.id}/profile`, {
  method: 'PUT',
}, { immediate: false });
const { execute: executeSIUsers } = useDataApi<SystemImplementationUser[]>(`/api/oscal/system-security-plans/${route.params.id}/system-implementation/users`, {
  method: 'GET',
}, { immediate: false });
const { execute: executeSIComponents } = useDataApi<SystemComponent[]>(`/api/oscal/system-security-plans/${route.params.id}/system-implementation/components`, {
  method: 'GET',
}, { immediate: false });
const { execute: executeSIInventory } = useDataApi<InventoryItem[]>(`/api/oscal/system-security-plans/${route.params.id}/system-implementation/inventory-items`, {
  method: 'GET',
}, { immediate: false });
const { execute: executeSILeveragedAuths } = useDataApi<LeveragedAuthorization[]>(`/api/oscal/system-security-plans/${route.params.id}/system-implementation/leveraged-authorizations`, {
  method: 'GET',
}, { immediate: false });
const { execute: executeDownloadJSON } = useDataApi(`/api/oscal/system-security-plans/${route.params.id}/full`, {
  method: 'GET',
}, { immediate: false });
...
watch(profiles, () => {
  profileItems.value = profiles.value?.map((item) => {
    return {
...
const selectedProfile = ref<{ name: string, value: string }>({ name: '', value: '' });
...
async function downloadJson(): Promise<void> {
  ...
  try {
    ...
    const dataStr = JSON.stringify(decamelizeKeys(data.value!, {separator: "-", deep: true}), null, 2)
