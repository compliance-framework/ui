<template>
  ...
  <template v-else-if="error">
    <PageSubHeader class="text-red-500">Error loading System Security Plan: {{ error }}</PageSubHeader>
  </template>
...
const { data: systemSecurityPlan, isLoading, error } = useDataApi<SystemSecurityPlan>(`/api/oscal/system-security-plans/${toValue(id)}`)
watch(error, () => {
  if (error.value) {
    ...
    toast.add({
      ...
      detail: errorResponse.response?.data.errors.body || 'An error occurred while loading the System Security Plan.',
      life: 3000,
    })
