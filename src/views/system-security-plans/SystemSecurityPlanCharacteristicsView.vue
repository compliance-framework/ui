<template>
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 border border-ccf-300 dark:border-slate-700 rounded-lg p-6">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-slate-300">System Characteristics</h2>
      <p class="text-gray-600 dark:text-slate-400 mt-1">Overview of system architecture, boundaries, and security characteristics</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-lg border border-indigo-200 dark:border-indigo-800">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{{ characteristics?.securitySensitivityLevel || 'N/A' }}</div>
            <div class="text-sm text-indigo-600 dark:text-indigo-400">Security Level</div>
          </div>
      ...
      <div class="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ authorizationStatus }}</div>
            <div class="text-sm text-green-600 dark:text-green-400">Authorization Status</div>
            <div v-if="daysSinceAuthorization !== null" class="text-xs text-green-500 dark:text-green-300 mt-1">
              {{ daysSinceAuthorization }} days ago
            </div>
          </div>
          <svg class="w-8 h-8 text-green-300 dark:text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ totalDiagrams }}</div>
            <div class="text-sm text-blue-600 dark:text-blue-400">Architecture Diagrams</div>
      ...
      <div class="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg border border-purple-200 dark:border-purple-800">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ completenessPercentage }}%</div>
            <div class="text-sm text-purple-600 dark:text-purple-400">Profile Completeness</div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-ccf-300 dark:border-slate-700 rounded-lg overflow-hidden">
      <div class="bg-gradient-to-r from-blue-500 to-indigo-600 p-6">
        <div class="flex justify-between items-start">
          <div>
            <h3 class="text-2xl font-bold text-white mb-2">{{ characteristics?.systemName || 'System Information' }}</h3>
            <p v-if="characteristics?.systemNameShort" class="text-blue-100">{{ characteristics.systemNameShort }}</p>
          </div>
          <button
            @click="() => {}"
      ...
      <div class="p-6">
        <div v-if="characteristics" class="space-y-6">
          <div v-if="characteristics.description" class="prose prose-sm dark:prose-invert max-w-none">
            <h4 class="text-sm font-semibold text-gray-700 dark:text-slate-400 uppercase tracking-wider mb-2">System Description</h4>
            <p class="text-gray-900 dark:text-slate-300 leading-relaxed">{{ characteristics.description }}</p>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div v-if="characteristics.securitySensitivityLevel" class="bg-gray-50 dark:bg-slate-800 rounded-lg p-4">
              <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider mb-2">Security Sensitivity Level</label>
              <p class="text-lg font-medium text-gray-900 dark:text-slate-300">{{ characteristics.securitySensitivityLevel }}</p>
            </div>

            <div v-if="characteristics.dateAuthorized" class="bg-gray-50 dark:bg-slate-800 rounded-lg p-4">
              <label class="block text-xs font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider mb-2">Date Authorized</label>
              <p class="text-lg font-medium text-gray-900 dark:text-slate-300">{{ formatDate(characteristics.dateAuthorized?.toString()) }}</p>
            </div>
          </div>

          <div v-if="characteristics.remarks" class="border-t border-gray-200 dark:border-slate-700 pt-4">
            <h4 class="text-sm font-semibold text-gray-700 dark:text-slate-400 uppercase tracking-wider mb-2">Additional Remarks</h4>
            <p class="text-gray-700 dark:text-slate-300 italic">{{ characteristics.remarks }}</p>
          </div>
        </div>
        ...
        <div v-else-if="loading" class="text-center py-8">
          <div class="inline-flex items-center">
            ...
            <span class="text-gray-600 dark:text-slate-400">Loading system characteristics...</span>
          </div>
        </div>

        <div v-else-if="error" class="rounded-lg bg-red-50 dark:bg-red-900/20 p-4">
        ...
        <div v-else class="text-center py-8">
          ...
          <p class="mt-2 text-gray-500 dark:text-slate-400">No system characteristics available</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white dark:bg-slate-900 border border-ccf-300 dark:border-slate-700 rounded-lg overflow-hidden">
        <div class="bg-gradient-to-r from-green-500 to-teal-600 p-4">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-white">Network Architecture</h3>
        ...
        <div class="p-4">
          <div v-if="networkArchitecture">
            <div v-if="networkArchitecture.description" class="mb-4">
              <p class="text-sm text-gray-700 dark:text-slate-300">{{ networkArchitecture.description }}</p>
            </div>

            <div v-if="networkArchitecture.diagrams?.length" class="space-y-2">
              <div class="flex items-center text-sm text-gray-600 dark:text-slate-400 mb-2">
              ...
              >
                <div class="font-medium text-sm text-gray-900 dark:text-slate-300">
                  {{ diagram.caption || `Network Diagram ${index + 1}` }}
                </div>
                <div v-if="diagram.description" class="text-xs text-gray-600 dark:text-slate-400 mt-1">
                  {{ diagram.description }}
                </div>
              </div>
            </div>
          ...
          <div v-else-if="networkArchitectureLoading" class="flex items-center justify-center py-4">
          ...
          <div v-else class="text-center py-4">
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">No network architecture defined</p>
          </div>
        </div>
      </div>
        ...
        <div class="p-4">
          <div v-if="dataFlow">
            <div v-if="dataFlow.description" class="mb-4">
              <p class="text-sm text-gray-700 dark:text-slate-300">{{ dataFlow.description }}</p>
            </div>
          ...
          <div v-else-if="dataFlowLoading" class="flex items-center justify-center py-4">
          ...
          <div v-else class="text-center py-4">
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">No data flow defined</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-900 border border-ccf-300 dark:border-slate-700 rounded-lg overflow-hidden">
        <div class="bg-gradient-to-r from-purple-500 to-pink-600 p-4">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-white">Authorization Boundary</h3>
            <svg class="w-6 h-6 text-white/80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-4">
          <div v-if="authorizationBoundary">
            <div v-if="authorizationBoundary.description" class="mb-4">
              <p class="text-sm text-gray-700 dark:text-slate-300">{{ authorizationBoundary.description }}</p>
            </div>

            <div v-if="authorizationBoundary.diagrams?.length" class="space-y-2">
              <div class="flex items-center text-sm text-gray-600 dark:text-slate-400 mb-2">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                </svg>
          ...
          <div v-else-if="authorizationBoundaryLoading" class="flex items-center justify-center py-4">
          ...
          <div v-else class="text-center py-4">
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">No boundary defined</p>
          </div>
        </div>
      </div>
    </div>
...
const totalDiagrams = computed(() => {
  let count = 0
  if (networkArchitecture.value?.diagrams?.length) count += networkArchitecture.value.diagrams.length
  if (dataFlow.value?.diagrams?.length) count += dataFlow.value.diagrams.length
  if (authorizationBoundary.value?.diagrams?.length) count += authorizationBoundary.value.diagrams.length
  return count
})
