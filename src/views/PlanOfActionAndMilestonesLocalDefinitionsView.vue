<template>
  <div class="p-6">
    ...
    <div v-if="loading" class="text-center py-8">
      <p class="text-gray-500 dark:text-slate-400">Loading local definitions...</p>
    </div>

    <div v-else-if="error" class="text-center py-8">
      <p class="text-red-500">Error loading local definitions: {{ error }}</p>
    </div>

    <div v-else class="text-center py-8">
      <p class="text-gray-500 dark:text-slate-400">No local definitions available.</p>
    </div>
    ...
    <div class="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md">
      <p class="text-sm text-yellow-800 dark:text-yellow-200">
        <strong>Read-Only Mode:</strong> Local Definitions functionality is currently disabled. The backend endpoint is not yet implemented.
      </p>
    </div>
  </div>
</template>
...
async function loadLocalDefinitions() {
  ...
  try {
    loading.value = true
    await poamStore.getLocalDefinitions(id)
  } catch (err) {
